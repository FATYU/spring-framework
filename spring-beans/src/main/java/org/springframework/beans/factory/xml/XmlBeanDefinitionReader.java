...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		if (currentResources == null) {
			...
			this.resourcesCurrentlyBeingLoaded.set(currentResources);//???? currentResources is null,将引用放入ThreadLocal中
		}
		if (!currentResources.add(encodedResource)) {//将资源对象添加到本地线程中
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource) throws BeanDefinitionStoreException {
		...
		try {
			...
			//NT:通过doc和resource进行beanDefinition注册
	...
	protected int detectValidationMode(Resource resource) {
		...
		try {
			...
			throw new BeanDefinitionStoreException(
					"Unable to determine validation mode for [" + resource + "]: an error occurred whilst reading from the InputStream.", ex);
		}
	}
